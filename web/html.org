...
================================================================================
        ...
        - 概念
          ...
          - 作色器
            - 分类
              - 顶点顶点作色器
                原始坐标转换为缩放空间坐标
                变量(varyings)/属性(attributes)
                gl_Position/aVertexPosition/uProjectionMatrix/uModelMatrix
              - 片段作色器
                顶点作色器处理完后，被要绘制得每个图形的每个像素点调用一次。
                确定像素的颜色，获取纹理元素的颜色，将适当的关照应用于颜色，保持到gl_FragColor中
                
        - 教程
          - Getting started with WebGL
            WebGL代码: 1) JavaScript控制代码; 2) GLSL(Shading Language)作色代码
            + 准备3D渲染
              #+BEGIN_SRC html
              <body onload="main()">
          ...
          - Adding 2D content to a WebGL context
            + 作色器
              ...
              #+END_SRC
            + 初始化作色器
              将定义的两个作色器传递给WebGL，编译并连接。
              #+BEGIN_SRC js
              // 初始化作色器程序，让WebGL知道如何绘制我们的数据
              ...
              const programInfo = {
                  ...
                  attribLocations: {
                      vertexPosition: gl.getAttribLocatioin(shaderProgram, 'aVertexPosition'),
                  },
                  uniformLocations: {
            ...
            + 绘制场景
              ...
              function drawScene(gl, programInfo, buffers){
                  ...
                  gl.enable(gl.GEPTH_TEST); // Enable depth testing
                  ...
                  /*
                   ...
                   ,* ratio that matches the display sizeof the canvas
                   ,* and we only want to see objexts between 0.1 units
  ...
  - JavaScript 功能与行为(动)
    - 概述
      - First-Class function(头等行数) 函数作为变量，参数，返回值；
        #+BEGIN_SRC js
        // 函数作为变量
        const foo = function(){
            console.log("foobar");
        ...
        #+END_SRC
      - Higher-Order Function(高阶函数) 返回函数的函数；
        #+BEGIN_SRC js
        function higherOrderSayHello(){
            return function(){
                console.log("Hello!");
            }
        }
        ...
        higherOrderSayHello()();
        // caller 2
        const sayHello = higherOrderSayHello();
        sayHello();
        #+END_SRC
      - 基于原型编程
        - 基于类和基于原型的区别
        ...
        - Employee
          - Employ tree
            ...
            - Manager
              ...
              SalesPerson.prototype = Object.cretae(WorkerBee.prototype);
              ...
              /* obj = new ObjName; 操作符创建对象细节
               ,* 1. 创建一个普通对象; obj = {}；
               ,* 2. 将对象的 prototype 指向对象的 prototype; obj.prototype = ObjName.prototype;
               ,* 3. 调用构造器； obj = obj.prototype.constructor(this);
               ,*/
              ...
              mark.bonus = 3000;
              // 添加全局属性
              Employee.prototype.specialty = "none"; // 所有employee实例同时具备specialy属性

              // 判断实例关系
              var e = new Employee();
              var isTrue = (e instanceof Employee);
              ...
              function Employee1(name, dept){
                  ...
                  this.id = idCounter++; // 继承时可能存在问题， 使用Object.create(Parent.prototype)解决
              }
              #+END_SRC
            - WorkerBee
              - SalesPerson
      ...
      - 面向对象
        #+BEGIN_SRC js
        // 命名空间
        var MYAPP = MYAPP || {}; // 首先检查是否已定义APP，如果未定义则定义MYAPP为{}
        // 子命名空间
        MYAPP.event = {
        ...
        function Person(firstName){
            this.firstName = firstName; // this作为当前对象的引用
            alert('Persion instantiated');
        }
        // 类方法(全局)
        ...
        helloFunction(); // Hello, I'm undefined, 因为this为null
        helloFunction.call(person1); // Hello, I'm Alice, call/apply函数可以指定this对象
        //--------------------------------------------------------------------------------
        // 继承
        // 1. 利用Parent.call(this, ...) 调用父类构造器
        // 2. 必须调用Object.create(Parent.prototype)
        // 3. 必须手动设置 Child.prototype.constructor = Child

        // 1. 定义子类
        function Student(firstName, subject){
            Person.call(this, firstName); // 利用call()来复用父类构造器
    ...
    - 变量
      ...
      - 声明
        ...
        - let 局部
          ...
          console.log(y); // ReferenceError: y
          #+END_SRC
        - const 只读
      - undefined
        如果变量未赋值其值为 undefined, 访问时抛出引用错误；
        undefined 可被转换为 false/NaN/null
        #+BEGIN_SRC js
        var input;
        if(input === undefined){
            // ...
    ...
    - 函数
      - 闭包
        - 可以自己拥有独立环境与变量的表达式
        - 内部行数只可以在外部行数中访问
        - 内部行数形成的闭包中，可以访问外部函数的参数和变量，但外部函数不能访问嵌套函数的变量和参数
          #+BEGIN_SRC js
          function outside(x){
              function inside(y){
                  return x + y; /* x 是外部函数参数 */
              }
              return inside; /* 返回内部函数 */
          }

          result = outside(3)(5); // 8 = 3(闭包参数) + 5(内部参数)
    ...
    - Using Promise
      ...
      - 链式调用(chaining)
        ...
        doSomething().then(function(result){
            ...
            console.log('Got the final result: ' + finalResult);
        }).catch(failureCb); // then(null, failureCb) 的缩略形式
        ...
        doSomething()
            ...
            .then(finalResult => {
                console.log(`Got the fianl result: ${fianlResult}`);
            }).catch(failureCb);


        // Catch 后继续执行其他操作
        // 遇到异常promise链的下一个onRejected为止
        new Promise((resolve, reject) => {
            console.log('1. init');
            resolve();
        }).then(() => {
            /* resolve 函数 */
            throw new Error('2. something error');
            console.log('2.1 never do');
        }).catch(() => {
            /* reject2 */
            console.log('3. on error');
        }).then(() => {
            console.log('3.1 never do');
        });
        ...
        #+END_SRC
      - 拒绝事件
        1. 事件派发到window/Worker
        2. rejectionhandled/unhandledrejection
           - PromiseRejectionEvent.promise 驳回的promise,便于提供补偿处理，方便调试
           - PromiseRejectionEvent.reason; 驳回原因
           - example
             #+BEGIN_SRC js
             // Node.js
             window.addEventListener("unhandledrejection", event => {
                 console.log(` ${event.promise} ${event.reason}`);
                 event.preventDefault(); //不要执行默认操作
             }, false);
             #+END_SRC
      - 改造timer,创建Promise
        包裹旧的API
        ...
        wait(10000).then(() => saySomething('10 seconds')).catch(failCallback);
        #+END_SRC
      - 组合
        - Promise.resolve()
        - Promise.reject()
      ...
      - 方法
        ...
        - Promose.reject(reason)
        - Promose.resolve(value)
      - 原型
      ...
      - example
        ...
        // promise模式
        const promise = createAudioFileAsyncPromise(audioSettings);
