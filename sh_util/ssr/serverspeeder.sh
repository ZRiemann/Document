...
Get_Dist_Name()
{
    if grep -Eqi "CentOS" /etc/issue || grep -Eq "CentOS" /etc/*-release; then
        release='CentOS'
        PM='yum'
	if [ "$(rpm -qa | grep kernel-3.10.0-229.1.2.el7.x86_64)" = "kernel-3.10.0-229.1.2.el7.x86_64" ]; then
	    echo "check kernel-3.10.0-229.1.2.el7.x86_64 ok"
	else
	    rpm -ivh http://soft.91yun.org/ISO/Linux/CentOS/kernel/kernel-3.10.0-229.1.2.el7.x86_64.rpm --force
	    echo "change kernel--3.10.0-229.1.2.el7.x86_64 done"
	    echo "reboot CentOS  now"
	    reboot
	fi
